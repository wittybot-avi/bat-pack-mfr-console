...
import { 
  Menu, Bell, Search, Users, LogOut, Zap, Database, Monitor
} from 'lucide-react';
...
import { DiagProbe } from './DiagProbe';
...
export const Layout = () => {
  ...
  const effectiveRole = currentRole || ROLES.find(r => r.id === 'SUPER_ADMIN') || ROLES[0];
  const effectiveCluster = currentCluster || CLUSTERS[effectiveRole.clusterId];
  ...
  const renderNavGroup = (groupName: string, screenIds: ScreenId[]) => {
    ...
    const visibleItems = screenIds.filter(id => id && canView(effectiveCluster.id, id));
  ...
  if (!effectiveRole || !effectiveCluster) return (
  ...
  const isSuperUser = effectiveCluster.id === 'CS';
        ...
        <div className="p-4 border-t dark:border-slate-800 shrink-0">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex flex-col overflow-hidden">
              <span className="text-sm font-medium truncate" title={effectiveRole.name}>{effectiveRole.name}</span>
              <span className="text-xs text-slate-500 truncate">{effectiveCluster.id} Cluster</span>
            </div>
          </div>
      ...
      <div className={`flex-1 flex flex-col transition-all duration-300 ${sidebarOpen ? 'pl-64' : 'pl-0'}`}>
        <header className="h-16 border-b bg-white dark:bg-slate-900 dark:border-slate-800 sticky top-0 z-40 px-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button 
              onClick={() => {
                const diag = localStorage.getItem('DIAG_ENABLED') === '1';
                localStorage.setItem('DIAG_ENABLED', diag ? '0' : '1');
