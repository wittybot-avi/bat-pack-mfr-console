...
export default function EolDetails() {
  ...
  const handleApprove = async () => {
    ...
    try {
        ...
        navigate(ROUTES.PROVISIONING_QUEUE);
    } catch (e: any) {
  ...
  return (
    <div className="pb-12 animate-in fade-in duration-500">
      <StageHeader 
        ...
        diagnostics={{ route: ROUTES.EOL_DETAILS, entityId: currentPack.id }}
      />

      <div className="max-w-7xl mx-auto px-6 space-y-6">
        <div className="flex items-center gap-4 mb-2">
            <Button variant="ghost" size="sm" onClick={() => navigate(ROUTES.EOL_QUEUE)} className="gap-2 text-slate-500">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <Card>
                <CardHeader className="border-b bg-slate-50/50 flex flex-row items-center justify-between">
                    ...
                    {!isPass && currentPack.status !== PackStatus.QUARANTINED && (
                        <Button size="sm" onClick={() => navigate(ROUTES.EOL_RUN.replace(':buildId', currentPack.id))} className="gap-2 bg-indigo-600 font-black text-[10px] uppercase tracking-wider h-8 shadow-md">
          ...
          <div className="space-y-6">
            <Card className="bg-slate-900 text-white border-none shadow-xl">
                ...
                <CardContent className="space-y-4 pt-6">
                    ...
                    {currentPack.batteryRecordCreated && (
                        <Button onClick={() => navigate(ROUTES.PROVISIONING_QUEUE)} className="w-full h-14 bg-indigo-600 hover:bg-indigo-700 border-none font-black text-xs uppercase gap-2 shadow-lg shadow-indigo-500/20">
                            <Cpu size={18} /> Provisioning Queue
                        </Button>
                    )}
                    <Button variant="outline" className="w-full text-white border-slate-700 hover:bg-slate-800 h-12 font-bold text-xs uppercase" onClick={() => navigate(ROUTES.EOL_QUEUE)}>
                        Exit Analysis
                    </Button>
                </CardContent>
            </Card>
            ...
            <div className="p-4 border-2 border-dashed rounded-xl bg-slate-50 dark:bg-slate-900/50 flex flex-col items-center justify-center text-center space-y-2 opacity-60">
                 ...
                 <Button variant="link" size="sm" className="text-[10px] font-bold" onClick={() => navigate(ROUTES.EOL_AUDIT.replace(':buildId', currentPack.id))}>Open Audit Node</Button>
            </div>
          </div>
        </div>
      </div>
